...
static ssize_t store_UV_mV_table(struct cpufreq_policy *policy,
	...
	ret = sscanf(buf, "%d %d %d %d %d %d %d %d", &exp_UV_mV[0], &exp_UV_mV[1], &exp_UV_mV[2], &exp_UV_mV[3], &exp_UV_mV[4], &exp_UV_mV[5], &exp_UV_mV[6], &exp_UV_mV[7], &exp_UV_mV[8], &exp_UV_mV[9]);
	if(ret != 1) {
		return -EINVAL;
	}
	else
...
static ssize_t store_states_enabled_table(struct cpufreq_policy *policy, const char *buf, int count) {
	...
	ret = sscanf(buf, "%d %d %d %d %d %d %d %d", &enabled_freqs[0], &enabled_freqs[1], &enabled_freqs[2], &enabled_freqs[3], &enabled_freqs[4], &enabled_freqs[5], &enabled_freqs[6], &enabled_freqs[7], &enabled_freqs[8], &enabled_freqs[9]);
	if(ret != 1) {
		return -EINVAL;
	}
	else
